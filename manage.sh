...
log_msg() {
    ...
    # Log to file
    if mkdir -p "$(dirname "$LOG_FILE")"; then echo "$entry" >> "$LOG_FILE"; chown "${TARGET_USER}:${TARGET_USER}" "$LOG_FILE" 2>/dev/null; chmod 640 "$LOG_FILE" 2>/dev/null; else echo "[$ts] ERROR: Cannot write to log $LOG_FILE" >&2; fi
    # Print to screen
...
confirm_action() {
    ...
    if [[ "$confirm" =~ ^[YyЕе]$ ]]; then return 0; else log "Action cancelled."; return 1; fi
}
...
check_dependencies() {
    log "Checking dependencies..."; local critical_error=0
    local dependencies=( "$AWG_DIR" "$CONFIG_FILE" "$PYTHON_VENV_PATH" "$PYTHON_EXEC" "$AWGCFG_SCRIPT_PATH" "$SERVER_CONF_FILE" )
    for dep in "${dependencies[@]}"; do if [ ! -e "$dep" ]; then log_error " - Missing: $dep"; critical_error=1; fi; done
    if [ -f "$PYTHON_EXEC" ] && [ ! -x "$PYTHON_EXEC" ]; then log_error " - Not executable: $PYTHON_EXEC"; critical_error=1; fi
    if [ -f "$AWGCFG_SCRIPT_PATH" ] && [ ! -x "$AWGCFG_SCRIPT_PATH" ]; then log_error " - Not executable: $AWGCFG_SCRIPT_PATH"; critical_error=1; fi
    if ! command -v awg &>/dev/null; then log_error " - Command 'awg' not found."; critical_error=1; fi
    if ! command -v qrencode &>/dev/null; then log_warn " - Command 'qrencode' not found (QR codes won't update on 'modify')."; fi
    if [ "$critical_error" -eq 1 ]; then die "Missing critical dependencies."; fi
    log "Dependencies OK.";
}
run_awgcfg() {
...
run_awgcfg_generate_clients() {
    ...
    if [ -f "$CONFIG_FILE" ]; then mv "$CONFIG_FILE" "$temp_conf_backup" || { log_warn "Workaround: Failed to move $CONFIG_FILE."; mv_failed=1; }; fi
    log_debug "Workaround mv_failed=$mv_failed"
    if ! run_awgcfg -c -q; then log_error "awgcfg.py -c -q failed."; result=1; else log_debug "awgcfg.py -c -q completed."; fi
    if [ "$mv_failed" -eq 0 ] && [ -f "$temp_conf_backup" ]; then mv "$temp_conf_backup" "$CONFIG_FILE" || log_error "Workaround FATAL: Failed to restore $CONFIG_FILE!"; fi
    rm -f "$temp_conf_backup"
    if [ ! -f "$CONFIG_FILE" ]; then log_error "$CONFIG_FILE is missing after client generation!"; if [ "$result" -eq 0 ]; then result=1; fi; fi
    # Ensure permissions again
    chown "${TARGET_USER}:${TARGET_USER}" "$AWG_DIR"/*.conf "$AWG_DIR"/*.png &>/dev/null || true
    find "$AWG_DIR" -maxdepth 1 -name "*.conf" -type f -exec chmod 600 {} \; &>/dev/null
    find "$AWG_DIR" -maxdepth 1 -name "*.png" -type f -exec chmod 644 {} \; &>/dev/null
    return $result
}
...
usage() {
    exec >&2; echo ""; echo "AmneziaWG Management Script (v2.2)"; echo "=================================="; echo "Usage: sudo bash $0 [OPTIONS] <COMMAND> [ARGS]"; echo "Options:"; echo "  -h, --help         Help"; echo "  -v, --verbose      Verbose 'list'"; echo "  --no-color         Disable color"; echo "  --conf-dir=PATH    AWG dir (Default: $DEFAULT_AWG_DIR)"; echo "  --server-conf=PATH WG server config (Default: $DEFAULT_SERVER_CONF_FILE)"; echo ""; echo "Commands:"; echo "  add <name>           Add client (+regen, requires restart)"; echo "  remove <name>        Remove client (requires restart)"; echo "  list [-v]            List clients"; echo "  regen                Regenerate all client .conf/.png files"; echo "  modify <n> <p> <v>   Modify client param (DNS, AllowedIPs...)"; echo "  check | status       Check server health"; echo "  show                 Run 'awg show'"; echo "  restart              Restart awg-quick@awg0 service"; echo "  help                 This help"; echo ""; echo "Restart required after 'add'/'remove': sudo bash $0 restart"; echo "Log: $LOG_FILE"; echo ""; exit 1;
}
